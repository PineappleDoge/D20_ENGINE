...
local CHAT_BUBBLE_FONT_SIZE = 16
local CHAT_BUBBLE_LINE_HEIGHT = CHAT_BUBBLE_FONT_SIZE + 10
...
local function createPlayerChatLine(player, message, isLocalPlayer)
	...
	if player then
		...
		this.Origin = shared.Framework.Players[player.userId].characterObject
	end
...
function recurse(v, pri, sec)
	if v.ClassName:find("Image") then
		v.BackgroundColor3 = pri
		v.ImageColor3 = pri
	elseif v.ClassName:find("Text") then
		v.BackgroundColor3 = pri
		v.TextColor3 = sec
		v.TextStrokeColor3 = shared.Framework.Util.Colors:shadow(pri, sec)
		v.TextStrokeTransparency = .5
	elseif v.ClassName:find("Frame") then
		v.BackgroundColor3 = pri
	end
...
local function decorateInstance(player, v)
	local pri = player.primary
	local sec = player.secondary
	recurse(v, pri, sec)
end
...
local function createBillboardInstance(adornee)
	...
	local smallTalkBubble = this:CreateSmallTalkBubble(BubbleColor.WHITE)
...
local function isPartOfLocalPlayer(adornee)
	if adornee and shared.Framework.Players[game.Players.LocalPlayer.userId].characterObject then
		return adornee:IsDescendantOf(shared.Framework.Players[game.Players.LocalPlayer.userId].characterObject)
	end
end
...
function this:CreateBubbleText(message)
	...
	bubbleText.TextSize = CHAT_BUBBLE_FONT_SIZE
...
function this:CreateChatLineRender(instance, line, onlyCharacter, fifo)
	...
	local player
	for i,v in next, shared.Framework.Players:getChildren() do
		if v.characterObject == instance then
			player = v
		end
	end
	...
	if billboardGui then
		...
		local currentTextBounds = TextService:GetTextSize(
				bubbleText.Text, CHAT_BUBBLE_FONT_SIZE, CHAT_BUBBLE_FONT,
		...
		local numOflines = (currentTextBounds.Y/CHAT_BUBBLE_FONT_SIZE)
		...
		if player then
			decorateInstance(player, billboardGui)
		end
...
function this:OnPlayerChatMessage(sourcePlayer, message, targetPlayer)
	...
	if sourcePlayer and line.Origin then
		...
		this:CreateChatLineRender(shared.Framework.Players[sourcePlayer.userId].characterObject, line, true, fifo)
	end
end

function this:OnGameChatMessage(origin, message, color)
	local localPlayer = PlayersService.LocalPlayer
	local fromOthers = localPlayer ~= nil and (shared.Framework.Players[localPlayer.userId].characterObject ~= origin)
...
function this:BubbleChatEnabled()
	return shared.Framework.ClientController.paused:ready()
end
