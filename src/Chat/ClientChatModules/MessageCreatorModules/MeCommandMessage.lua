...
function CreateMeCommandMessageLabel(messageData, channelName)
	...
	local function UpdateTextFunction(messageObject)
		local fromSpeaker = messageObject.FromSpeaker
	
		if game.Players:FindFirstChild(fromSpeaker) then
		...
		if messageData.IsFiltered then
			BaseMessage.Text = string.rep(" ", numNeededSpaces) .. fromSpeaker .. " " .. string.sub(messageObject.Message, 5)
		else
			local messageLength = string.len(fromSpeaker) + messageObject.MessageLength - 4
