...
function State:update(dt, character)
	...
	local vely = character.physics.Velocity.Y
	vely = math.max(vely, Framework.Util.Vectors:hor(character.physics.Velocity, 0).magnitude/character.speed * -character.gravity/10)
	character.physics.Velocity = Framework.Util.Vectors:hor(character.physics.Velocity, vely)
